...
After setting up the system, the user can test the BigQuery library using the Example services. Thus, user can do all sorts of things to the accounts they have access to.
...
#### Example
Services: All services are with-respect-to a project in gcloud. All the services below shows how to - 

  * `BQExampleDeleteDataset`: delete a dataset.
  * `BQExampleGetDataset`: get a particular dataset.    
  * `BQExampleInsertDataset`: insert a dataset.
  * `BQExampleInsertAllRows`: insert rows in a table of dataset.
  * `BQExampleListAllDataset`: list all datasets.
  * `BQExampleUpdateDataset`: updates(generally replaces) an existing dataset.

### Code Libraries
`BigQueryLib` - This library helps user get started with the BigQuery & perform basic operations

### Portals
`BigQueryPortal`: Gives a demo how to use the bigquery library.
